...
Mss = pm.monkey_weight * grans_density
Mss_EGFP = Mss * pm.EGFP_ratio # * 0.4
#print EGFP_ratio, Mss, Mss_EGFP

num_appear_EGFP_clones = 550
...
############### load parameters ####################
shrink_ratio = .3
tot_time = (pm.months[-1] * 30 + 1) * shrink_ratio
dt = 0.1

thres_num_detect = Mss_EGFP/S_EGFP
...
def get_expr_data(CELL_TYPE = 'Grans', SKIP_MONTH2 = True, ABUND_EMPTY_ROW = True):
    input_all = pd.read_csv('mmc6_monk3.txt', sep='\t')
    ...
    rowNum_grans = input_all[columns_grans].shape[0] # default setting
    for i in range(rowNum_grans):
        if input_all.loc[i,columns_grans][0] != input_all.loc[i,columns_grans][0]:
            rowNum_grans = i
            break

    data_grans_origin = input_all.loc[0:rowNum_grans-1, columns_grans]
    if SKIP_MONTH2:
        data_grans = data_grans_origin[columns_grans[1:]] # abandon sample at month 2
    else:
...
def show_pulses_ages(L, pulse_maxtime = 60, dt=dt):
    ...
    plt.plot(ts_p, ms_one_pulse, 'k', linewidth=3)
...
############### Simulation parameters ####################
def simulate_HSC_dynamics_withAging(f_hi, Ass, Hss_EGFP = 2000., mu_h=0, tot_HSC_dynamics_time=tot_time):
    curr_time = 0
    inj_times = []
    hi_recs = []
    event_times = []
    ...
    while curr_time < tot_HSC_dynamics_time and hi > 0:
        ...
        curr_time += log(1/(1-random.random()))/rate_all

        test_num = rate_all * random.random()
        for i in range(max_hi):
            if ages[i] == max_age:
                continue

            if test_num < (alp + mu_h + mu_h): # the i-th clone's business
                if test_num < alp: # inj
                    ages[i] += 1
                    inj_times.append(curr_time)
                elif test_num < alp + mu_h: # death
                    ages[i] = max_age
                    hi -= 1
                else: # self-renewal
                    ages[i] += 1
                    ages.append(ages[i]) # generate another same-age HSC

                    hi = sum([age != max_age for age in ages]) # active HSC num
                    max_hi += 1 # update the hi list
                break
            else:
                test_num -= (alp + mu_h + mu_h)
    return (inj_times, event_times, hi_recs)

def simulate_HSC_dynamics_wDetermine(f_hi, Ass, Hss_EGFP = 2000., mu_h=0, tot_HSC_dynamics_time=tot_time, RETURN_HI=False):
    ...
    while curr_time < tot_HSC_dynamics_time and hi > 0:
        ...
        rate_inj = alp * hi
        rate_birth = hi * mu_h #mu_h * 1 * (1 - curr_time/half_effect_time)
        #print rate_birth
        rate_death = hi * mu_h
        rate_all = rate_inj + rate_birth + rate_death
...
def simulate_freq(f_hi,
                  ...
                  type=1,
                  mu_d = mu_d_origin,
                  sample_num=len(pm.months)-1,
                  use_expr_months = True,
                  pulse_maxtime = 60):

    if use_expr_months:
        sample_months = pm.months[1:sample_num+1]
...
def config_plot(xlab='', ylab='', legend_loc=(0.,0.), legend_prop=18):
    ...
    plt.subplots_adjust(left=MARGIN_LEFT, bottom=MARGIN_BOTTOM,
                        right=MARGIN_RIGHT, top=MARGIN_TOP)#, wspace=0, hspace=0)
